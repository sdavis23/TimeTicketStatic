...
import {OnSiteButton, OnSiteLinkButton} from "./render_cell_utils.js";
...
export class DailyWorkTicketEditor extends Component
{
	...
	handleProjectChange(val)
	{
		...
		this.setState({
			dailyworkticket_id: this.state.dailyworkticket_id,
			current_date: this.state.current_date,
			job_number: selected_project.job_num,
			labour_rows: this.state.labour_rows,
			labour_delete: this.state.labour_delete,
			equipment_rows: this.state.equipment_rows,
			equipment_delete: this.state.deleted_rows,
			save_clicked: false,
			modal_open: false,
        ...
        const employee_table = <EmployeeTable
         	...
         	return(  	
         				...
         				<div >
	         				...
	         				<div key = {5}  style={{left: 800, position: "absolute", top: 2}}>

	         					<OnSiteLinkButton link = "/calendar" text = "Go To Calendar View" />
	         					
	         				</div>
								
	         				<div key = {1} style={{left: 250, position: "absolute", top: 675}}>
         						{renderSelection(job_numdropdown, this.props.dailyticket_summary.project.job_num, this.handleProjectChange)}
         					</div>

         					<div key = {2} style={{position: "absolute", top: 675}}>	
         						<DateTimePicker onChange = {this.handleDateChange} defaultValue={new Date(date_split[0], date_split[1] - 1, date_split[2])} format={"MM/DD/YYYY"} style={{positon: "absolute", left: 500, width: 250}} />
         					</div>
         				
         				
         					<TabbedComponent key = {3} tabs={tab_items} />

         					<Portal key ={4} isOpened = {true} >
         		 				<button  style={{position: "absolute", top: 675}} onClick=  {(e) => this.handleSaveClick()} >Save Tables</button>
         		 			</Portal>
         				 </div>
